...
class Trading212:
    ...
    def setup(self, username, password, panel):
        ...
        WebDriverWait(self.driver, self.timeout).until(
            expected_conditions.invisibility_of_element((By.ID, "platform-loader")))
        ...
        if self.mode == Mode.Invest:
            ...
            WebDriverWait(self.driver, self.timeout).until(
                expected_conditions.element_to_be_clickable(
                    (By.XPATH, "//div[@class='custom-button send-order-button']"))).click()
        else:
...
class Invest(Trading212):
    ...
    def __init__(self, username, password, panel=Panel.Practice, headless=True, long_sleep=0.5, short_sleep=0.1,
                 timeout=30):
        """
    ...
    def close_position(self, stock):
        ...
        script_click_xpath(self.driver,
                           f"//td[@class='name' and text()='{stock}']/following::div[@class='button-wrapper']/child::div[@class='sell-button']")
        elem = WebDriverWait(self.driver, self.timeout).until(
            expected_conditions.visibility_of_element_located((By.XPATH, "//div[@class='visible-input']//input")))
        elem.clear()
        ...
        WebDriverWait(self.driver, self.timeout).until(expected_conditions.element_to_be_clickable(
            (By.XPATH, "//div[@class='custom-button send-order-button']"))).click()
