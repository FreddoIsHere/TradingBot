...
def get_daily_data(ticker, compact=True):  # 1 call
    ...
    for period in indicator_periods:
        indicators.append(TA.SMA(ohlc=stock, period=period).to_frame(name='SMA'))
        indicators.append(TA.DEMA(ohlc=stock, period=period).to_frame(name='DEMA'))
        ...
        indicators.append(TA.BBWIDTH(ohlc=stock, period=period).to_frame(name='BBWIDTH'))  # volatility
